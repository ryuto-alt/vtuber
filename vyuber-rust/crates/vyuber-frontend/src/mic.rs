...
use vyuber_shared::chat::ChatMode; // ★追加: ChatModeをインポート
...
// ... (downsample, f32_to_le_bytes 関数は変更なし) ...
                                    ...
                                    if response.is_final && !response.text.is_empty() {
                                        ...
                                        if send_to_chat.get_untracked() && !content.is_empty() {
                                            ...
                                            // --- リクエスト1: ガヤ (8B / Swarm) ---
                                            {
                                                ...
                                                spawn_local(async move {
                                                    match crate::services::chat_api::send_message(&text_to_send, ChatMode::Swarm).await {
                                                        Ok(comments) => {
                                                        ...
                                                        Err(e) => error!("Swarm API error: {}", e),
                                                    }
                                                });
                                            }

                                            // --- リクエスト2: 固定ファン (70B / Anchor) ---
                                            {
                                                let text_to_send = content.to_string();
                                                let state_anchor = state.clone();
                                                spawn_local(async move {
                                                    match crate::services::chat_api::send_message(&text_to_send, ChatMode::Anchor).await {
                                                        Ok(comments) => {
                                                            for (i, comment) in comments.into_iter().enumerate() {
                                                        ...
                                                        Err(e) => {
                                                            error!("Anchor API error: {}", e);
                                                            state_anchor.add_message(ChatUser::Ai("System".to_string()), format!("エラー: {}", e), "text-red-400".to_string());
                                                        }
                                                    }
                                                });
                                            }
